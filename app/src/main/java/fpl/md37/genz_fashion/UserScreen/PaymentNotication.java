...
public class PaymentNotication extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Gson gson = new Gson();
        Type productListType = new TypeToken<List<ProducItem>>() {}.getType();
    ...
    private void navigateToFragment(MyOrderFragment fragment) {
        getSupportFragmentManager().beginTransaction().replace(R.id.container, fragment) // `R.id.container` là vùng chứa Fragment trong MainActivity
                .addToBackStack(null) // Lưu trạng thái để quay lại nếu cần
                .commit();
    }

    void order(String userId, String selectedPaymentMethod, ArrayList<ProducItem> products){
